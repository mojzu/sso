use crate::{
    driver::postgres::schema::auth_key, DriverResult, Key, KeyCount, KeyCreate, KeyList, KeyRead,
...
impl ModelKey {
    ...
    pub fn count(conn: &PgConnection, count: &KeyCount) -> DriverResult<usize> {
        ...
        match count {
            KeyCount::AllowToken(count_service_id, count_user_id) => auth_key
                ...
                .get_result::<i64>(conn),
            KeyCount::AllowTotp(count_service_id, count_user_id) => auth_key
                .select(sql::<BigInt>("count(*)"))
                .filter(
                    key_is_enabled
                        .eq(true)
