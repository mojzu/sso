use crate::{
    api_types::AuthOauth2CallbackRequest, AuditBuilder, Client, CoreError, CoreOauth2Error,
    CoreResult, Csrf, Driver, ServerOptionsProviderOauth2, Service,
};
use http::header;
use oauth2::{
...
pub fn github_oauth2_url(
    ...
    let client = github_client(provider)?;
...
fn github_client(provider: Option<&ServerOptionsProviderOauth2>) -> CoreResult<BasicClient> {
    ...
    let redirect_url = Url::parse(&provider.redirect_url).map_err(CoreError::UrlParse)?;
    Ok(BasicClient::new(
        github_client_id,
...
pub fn microsoft_oauth2_url(
    ...
    let client = microsoft_client(provider)?;
...
fn microsoft_client(provider: Option<&ServerOptionsProviderOauth2>) -> CoreResult<BasicClient> {
    let provider = provider.ok_or_else(|| {
        // Warn OAuth2 is disabled, return bad request error so internal server error
        // is not returned to the client.
        let err = CoreError::Oauth2(CoreOauth2Error::Disabled);
