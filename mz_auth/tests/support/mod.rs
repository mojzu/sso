...
lazy_static! {
    static ref CLIENT: ClientSync = {
        ...
        let authorisation = env_test_mz_auth_key();
        let options = ClientOptions::new(authorisation);
        let url = env_test_mz_auth_url();
        ClientSync::new(url, executor_options, options).unwrap()
    };
}

fn env_test_mz_auth_url() -> String {
    std::env::var("TEST_MZ_AUTH_URL")
        .expect("TEST_MZ_AUTH_URL is undefined, integration test disabled")
}

fn env_test_mz_auth_key() -> String {
    std::env::var("TEST_MZ_AUTH_KEY")
        .expect("TEST_MZ_AUTH_KEY is undefined, integration test disabled")
}
