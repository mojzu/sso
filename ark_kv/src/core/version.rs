...
pub fn list(driver: &Driver, key: &Key) -> Result<Vec<Version>, Error> {
    let now = Utc::now();
    let id_list = driver
...
pub fn read_by_id(driver: &Driver, id: &str) -> Result<Version, Error> {
    read_opt_by_id(driver, id).and_then(|x| x.ok_or_else(|| Error::Unwrap))
}

pub fn read_opt_by_id(driver: &Driver, id: &str) -> Result<Option<Version>, Error> {
...
pub fn delete_by_id(driver: &Driver, id: &str) -> Result<usize, Error> {
