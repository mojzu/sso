        ...
        server_oauth2_form_error!(&server, audit, &client, &template, async {
            match query {
                AuthPasswordResetParse::Accept(code) => {
                    ...
                    server.response_template(&client, TEMPLATE_AUTH_PASSWORD_RESET_ACCEPT_OK)
                }
                AuthPasswordResetParse::Reject(code) => {
                    ...
                    server.response_template(&client, TEMPLATE_AUTH_PASSWORD_RESET_REJECT_OK)
                }
            }
        })
    })
...
impl AuthPasswordResetParse {
    fn template_get(&self) -> &'static str {
        match self {
            Self::Accept(_) => TEMPLATE_AUTH_PASSWORD_RESET_ACCEPT,
            Self::Reject(_) => TEMPLATE_AUTH_PASSWORD_RESET_REJECT,
        }
    }
