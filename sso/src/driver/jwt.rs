//! # JSON Web Tokens
use crate::{csrf, DriverError, DriverResult, KeyWithValue, Postgres, Service, User, UserToken};
...
fn encode_csrf(
    ...
    let csrf = driver.csrf_create(&csrf::CsrfCreate::generate(exp, service_id))?;
...
pub fn encode_user(
    ...
    let (access_token, access_token_expires) = encode(
        service.id,
        user.id,
        JwtType::AccessToken,
        &key.value,
    ...
    let (refresh_token, refresh_token_expires) = encode_csrf(
        driver,
        service.id,
        user.id,
...
pub fn decode_refresh(
    ...
    csrf::verify(driver, service.id, csrf_key)?;
    Ok(())
}
...
pub fn decode_csrf(
    ...
    let (_, csrf_key) = decode(service.id, user.id, token_type, &key.value, &token)?;
    if let Some(csrf_key) = csrf_key {
        driver.csrf_read(&csrf_key)?;
    }
    Ok(())
}
