use crate::{
    DriverError, DriverResult, KeyType, BYTES_KEY_VALUE, MAX_AUDIT_SUBJECT, MAX_AUDIT_TYPE,
    MAX_NAME, MAX_OAUTH2, MAX_TEXT, MAX_TOTP, MAX_USER_LOCALE, MAX_USER_PASSWORD,
    MAX_USER_TIMEZONE, MIN_USER_PASSWORD,
};
...
impl Validate {
    ...
    pub fn password_opt(errors: &mut ValidationErrors, field: &'static str, value: Option<&str>) {
        if let Some(value) = value {
            Self::password(errors, field, value);
        }
    }
    ...
    pub fn locale_opt(errors: &mut ValidationErrors, field: &'static str, value: Option<&str>) {
        if let Some(value) = value {
            Self::locale(errors, field, value);
        }
    }
    ...
    pub fn timezone_opt(errors: &mut ValidationErrors, field: &'static str, value: Option<&str>) {
        if let Some(value) = value {
            Self::timezone(errors, field, value);
        }
    }
    ...
    pub fn audit_subject_opt(
        errors: &mut ValidationErrors,
        field: &'static str,
        value: Option<&str>,
    ) {
        if let Some(value) = value {
            Self::audit_subject(errors, field, value);
        }
    }

    pub fn audit_subject_vec(errors: &mut ValidationErrors, field: &'static str, value: &[String]) {
        for v in value {
            Self::audit_subject(errors, field, v);
        }
    }
    ...
    pub fn csrf_expires_s_opt(
        errors: &mut ValidationErrors,
        field: &'static str,
        value: Option<i64>,
    ) {
        if let Some(value) = value {
            Self::csrf_expires_s(errors, field, value);
        }
    }
