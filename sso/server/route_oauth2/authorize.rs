    ...
    server_request!(&server, &req, async {
        ...
        server_oauth2_error!(&server, audit, &client, template::ERROR, async {
            ...
            if let Some(id) = ident.identity() {
                ...
                server.response_template_context(&client, template::AUTH, context)
            }
        })
    })
}
...
pub async fn post(
    ...
    server_request!(&server, &req, async {
        ...
        server_oauth2_form_error!(&server, audit, &client, template::AUTH, async {
            match request {
                ...
                AuthorizeFormParse::PasswordReset(email) => {
                    ...
                    server.response_template(&client, template::AUTH_PASSWORD_RESET)
                }
                ...
                AuthorizeFormParse::Register(email) => {
                    ...
                    server.response_template(&client, template::AUTH_REGISTER)
                }
            }
        })
    })
