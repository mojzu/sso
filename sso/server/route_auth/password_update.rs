...
pub async fn get(
    ...
    server_request!(&server, &req, async {
        ...
        server_oauth2_error!(&server, audit, &client, template::ERROR, async {
            ...
            server.response_template_context(&client, template::AUTH_PASSWORD_UPDATE, context)
        })
    })
}
    ...
    server_request!(&server, &req, async {
        ...
        server_oauth2_form_error!(
            ...
            async {
                ...
                server.response_template(&client, template::AUTH_PASSWORD_UPDATE_OK)
            }
        )
    })
}
