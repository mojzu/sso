        ...
        server_oauth2_form_error!(&server, audit, &client, &template, async {
            match query {
                AuthRegisterQueryParse::Accept(code) => {
                    ...
                    match request {
                        AuthRegisterFormParse::Password(args) => {
                            ...
                            server.response_template(&client, template::AUTH_REGISTER_ACCEPT_OK)
                        }
                ...
                AuthRegisterQueryParse::Reject(code) => {
                    ...
                    server.response_template(&client, template::AUTH_REGISTER_REJECT_OK)
                }
            }
        })
    })
...
impl AuthRegisterQueryParse {
    fn template_get(&self) -> &'static str {
        match self {
            Self::Accept(_) => template::AUTH_REGISTER_ACCEPT,
            Self::Reject(_) => template::AUTH_REGISTER_REJECT,
        }
    }
