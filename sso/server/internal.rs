...
impl Server {
    ...
    pub(crate) fn template_context(&self, client: &Client) -> template::Context {
        template::Context::build(&self.config.oauth2.providers, client)
    }

    pub(crate) async fn template_csrf_context(
        &self,
        client: &Client,
    ) -> oauth2::Result<template::Context> {
        self.csrf_token(client).await.map(|token| {
            let mut context = template::Context::build(&self.config.oauth2.providers, client);
...
impl Server {
    pub async fn client_required(&self, auth: BasicAuth) -> HttpResult<Client> {
...
impl Server {
    ...
    pub(crate) async fn user_password_reset_request(
        ...
        self.mailto
            .send(MailtoSend::new(user_email, subject, &text))
            .await
            .map_err(|e| oauth2::ErrorResponse::server_error(&e.to_string()))?;
        Ok(())
    }
