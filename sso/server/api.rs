...
async fn ping(server: Data<Server>, req: HttpRequest) -> HttpResult<Json<String>> {
    server_request!(&server, &req, async { Ok(Json("ok".to_string())) })
}

#[api_v2_operation(summary = "Server health")]
async fn health(server: Data<Server>, req: HttpRequest) -> HttpResult<Json<String>> {
...
async fn metrics(server: Data<Server>, req: HttpRequest) -> HttpResult<HttpResponse> {
    server_request!(&server, &req, async { Ok(server.metrics()) })
}
